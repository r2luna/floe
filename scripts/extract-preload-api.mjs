...
//   - src/preload/api.ts   : buildRookeryApi(ipcRenderer, host) — pure, no electron/node
...
const exposeIdx = src.indexOf("contextBridge.exposeInMainWorld('rookery', api)")
...
export function buildRookeryApi(ipcRenderer: IpcLike, host: RookeryHost) {
  ${apiObj.replace(/\n/g, '\n  ')}
  return api
}
...
import { buildRookeryApi, type RookeryHost } from './api'
...
contextBridge.exposeInMainWorld('rookery', buildRookeryApi(ipcRenderer, host))
