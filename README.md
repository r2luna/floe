...
- **Right panel** — toggles between **Files** (tree + quick-open), **Review** (changed-files diff with inline comments), **Plans** (markdown saved under `.rookery/plans`), **Tasks** (the tracker's issues), and **PRs** (the repo's open pull requests — review, comment, approve & merge).
...
- **Remappable keys** — defaults live in `src/renderer/src/keybindings.ts`; user overrides in `~/.config/rookery/keybindings` layer on top (any command rebindable, `null` to unbind). Edit them in nvim straight from the palette.
