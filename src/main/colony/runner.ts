...
import { changedFiles, createWorktree, mergeWorktree, snapshotTree, undoMerge, type MergeResult } from '../git'
...
function finishLane(win: BrowserWindow, id: string, stage: string, text: string): void {
  ...
  if (!current || !task.worktreePath) return settleLane(win, id, stage, text)
...
function settleLane(win: BrowserWindow, id: string, stage: string, text: string, step?: StepRecord): void {
  const task = getTask(id)
  if (!task || task.stage !== stage || task.status !== 'working') return
